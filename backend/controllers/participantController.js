...
const user_permitted = async (inviting_user_id, sheet_id, session_token) => {
	...
	if (INVITING_USER_IS_PARTICIPANT) {
		return { message: INVITING_USER_IS_PARTICIPANT_MESSAGE };
	}
		...
		Participant.destroy({
			...
			.then((rows_deleted) => {
				if (rows_deleted === 0) {
					res.status(500).json({
						ok: false,
						error: "could not find participant to permitted to be removed. Participant may be owner",
					});
				} else {
					res.status(200).json({
						ok: true,
