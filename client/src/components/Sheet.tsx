...
const Sheet: React.FC<SheetProps> = ({ id, name, Participants }) => {
	return (
		<div className={"pb-3"}>
			<Link className={"flex flex-col gap-2 p-3 border-[1px] border-gray-400 rounded-sm"} to={`/sheets/${id}`}>
						...
						<div key={"participant" + participant.id} className={"flex items-end"}>
							...
							{participant.isOwner && (
								<FontAwesomeIcon
									icon={faCrown}
									className={
										"w-[14px] h-[14px] text-accent-default translate-x-[-10px] translate-y-[5px] " +
