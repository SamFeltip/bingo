...
export const Participant: React.FC<ParticipantComponentProps> = ({user, handle_invite}) => {
	...
	const participant_board = (
		<div className={'grid grid-cols-5 gap-[1px]'}>
			{
				participantSheetItems && participantSheetItems.map(participant_sheet_item => {
				return (
					<div
						...
						className={
							...
							' flex justify-center items-center text-center' + (
								(participant_sheet_item.checked)
	...
	return (
		<div className={'pb-2 flex flex-row items-center gap-2'}>
			{participant &&
				<div
					className={'border-[1px] rounded-sm border-primary-default aspect-square flex items-center justify-center ' +
						' h-[30px]'}>
					...
					Participants.length == 0
						? (
							...
							<button
								...
								onClick={() => {
									!participant.isOwner && handle_invite({id, name, image, Participants})
								}}
