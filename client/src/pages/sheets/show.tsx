	...
	const createParticipantSheetItems = (participant_id: string) => {
		axios({
			...
			.catch((error) => {
				...
				setNotification({
					type: NotificationMethods.Error,
					message: "unable to build a new sheet",
				});
			});
	};
	...
	const toggleParticipantBlock = (participantSheetItem: ParticipantSheetItemProps) => {
		fetch(process.env.REACT_APP_BACKEND_URL + "/participantSheetItems/" + participantSheetItem.id, {
			...
			.catch((err) => {
				...
				setNotification({
					type: NotificationMethods.Error,
					message: "could not send check to server",
				});
			});
	};

	const start_game_message = participant?.isOwner ? (
		<div className={"min-w-[300px] max-w-[700px] text-center"}>
			<div>invite friends to start the game!</div>
			<div>
				<Link
		...
		<div>The creator of this bingo game needs to create the bingo sheets before you can begin</div>
	);
