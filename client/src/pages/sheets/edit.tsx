...
export function EditSheet() {
	...
	useEffect(() => {
		axios({
			...
			.catch((error) => {
				...
				setNotification({
					type: NotificationMethods.Error,
					message: "could not retrieve sheet",
				});
			});
	}, []);
	...
	const handle_invite = (user: UserProps) => {
		...
		if (sheet_id) {
			...
			setNotification({
				type: NotificationMethods.Error,
				message: "no sheet id found in URL",
			});
		}
	};
	...
	return (
		<div className={"py-3 px-7"}>
			<div className={"pb-3"}>
				<h1 className={"font-bold text-xl pb-2"}>{sheet && sheet.name}</h1>
			...
			<div>
				{loading
					...
					: invitedUsers.map((user) => (
							<Participant key={user.id} handle_invite={handle_invite} user={user} />
					  ))}
